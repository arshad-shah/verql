...
| 10 | Toasts → Cynosure `Toaster`/Sonner (store + ToastContainer deleted; IPC bus via `lib/toast-ipc.ts`); notification items → Cynosure `Notification`. `theme-toggle` deliberately not adopted: binary context toggle vs Verql's settings-driven 3-way mode + registry themes (would fight `CynosureSchemeSync`) | ✅ done |
