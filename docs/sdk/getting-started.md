# Getting started: your first Verql plugin

A minimal, end-to-end plugin using [`@verql/plugin-sdk`](./README.md). We'll
build a tiny **exporter** (no special permissions needed), then point you at the
full surface catalogue in [`../plugins.md`](../plugins.md).
...
and the user must grant it. See [`../plugin-security.md`](../plugin-security.md).
...
- **By hand:** drop the folder into `<userData>/plugins/<name>/`
  ...
  [troubleshooting page](../guide/troubleshooting.md)).
...
- The capability model, the trust boundary, and what Verql does/doesn't
  enforce: [`../plugin-security.md`](../plugin-security.md).
