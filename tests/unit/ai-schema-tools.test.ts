...
const mockSchema: SchemaAccess = {
  getTables: vi.fn(async (_conn: string) => [{ name: 'users', type: 'table' as const, schema: 'public' }]),
  getColumns: vi.fn(async (_conn: string) => [
  ...
  getIndexes: vi.fn(async (_conn: string) => []),
  getSchemas: vi.fn(async (_conn: string) => ['public']),
  getDatabases: vi.fn(async (_conn: string) => ['mydb']),
  getSchemaSummary: vi.fn(async (_conn: string) => ({ tables: [] }))
}

describe('Schema tools', () => {
  it('creates listTables tool', () => {
    const tools = createSchemaTools(mockSchema)
    const listTables = tools.find(t => t.id === 'schema_list_tables')
  ...
  it('describeTable returns columns', async () => {
    const tools = createSchemaTools(mockSchema)
    const describeTool = tools.find(t => t.id === 'schema_describe_table')!
