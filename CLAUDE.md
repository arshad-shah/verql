...
**[`@arshad-shah/cynosure-react`](https://cynosure.arshadshah.com)** — all UI components come from Cynosure, styled exclusively through their props API (authority: the package's `.d.ts` files). See [`docs/cynosure-migration.md`](./docs/cynosure-migration.md) for the migration rules, the theming bridge, per-stage status, and the contract tests (`tests/unit/cynosure-*-migration.test.tsx`) that pin the behaviours call sites rely on. Never add a wrapper around a Cynosure component or re-implement a feature a component already ships (e.g. `Input type="password"` has a built-in reveal toggle).

Theming: Verql's theme system stays authoritative — `primitives/theme/tokens.css` (raw scale → semantic `--color-*` tokens) + `baseline.css` (brand Nightshift), themes from the `core-themes` plugin and third-party plugins applied via `data-theme`, managed by `ThemeProvider`. `styles/cynosure-bridge.css` aliases every semantic `--cynosure-*` token to the Verql tokens (coverage enforced by `tests/unit/cynosure-bridge.test.ts`), so any theme restyles Cynosure automatically; `primitives/theme/cynosure.tsx` mounts Cynosure's providers and publishes the active theme's light/dark type on `data-cynosure-scheme` (scrollbars, charts, CodeBlock highlighting).

The only surviving Verql components in `src/renderer/src/primitives/` carry app behaviour Cynosure doesn't model: `typography/KbdGroup` (Electron accelerator parsing → Cynosure `Kbd` caps), `forms/FileContentInput`/`FilePathInput` (native dialogs, drag-drop), `surfaces/GradientSurface` (brand gradient), `utilities/ResizeHandle` (settings-persisted shell layout; conversion to Cynosure `Resizable` panel groups is a deferred dedicated refactor), plus `theme/` and `utils/cn`. Component-level Tailwind classes still on call sites are the stage-11 long tail.
