...
outbound network requests, and general **diagnostic log lines** from the glue.
One stream feeds two audiences: **users** (a readable, filterable record of what
the app did) and **developers** (in-app diagnostics, no log files to dig out of
`userData`). It is also exposed to agents read-only via a shared tool.
...
title, detail, and source), **kind** and **level** chips (multi-select; empty =
show all), **pause / resume**, **export** (download the matching entries as
`verql-activity-<ts>.json`), and **clear**.
