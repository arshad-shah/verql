---
title: IPC channels
description: How the renderer and main process communicate through Verql's typed, centrally-registered IPC channels, and how to add a new invoke channel or broadcast event.
sidebar:
  order: 3
---
...
[`shared/ipc.ts`](https://github.com/arshad-shah/verql/blob/main/shared/ipc.ts). There are two kinds:
...
3. Implement the handler. Pick the right file under `src/main/ipc/` based
   ...
   - `connections:*` → [`ipc/connections.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/connections.ts)
   - `db:*` → [`ipc/db.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/db.ts)
   - `export:*` / `import:*` → [`ipc/export-import.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/export-import.ts)
   - `plugins:*` → [`ipc/plugins.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/plugins.ts)
   - `settings:*` → [`ipc/settings.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/settings.ts)
   - `dialog:*` → [`ipc/dialog.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/dialog.ts)
   - `keyring:*` → [`ipc/keyring.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/keyring.ts)
   - `mcp:*` → [`ipc/mcp.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/mcp.ts)
   - `migration:*` → [`ipc/migration.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/migration.ts)
   - `app:*` → [`ipc/app.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/app.ts)
   ...
   `handle` is the wrapper defined in [`ipc/context.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/ipc/context.ts).
