...
description: How Verql limits the blast radius of third-party plugins — the trust boundary, the capability/permission model, process isolation, install hardening, and known limitations.
sidebar:
  order: 1
---
...
[`architecture.md`](/develop/architecture/) for the plugin model first, then
[`plugins.md`](/plugins/) for the contribution surfaces. This doc is the
...
`activate(ctx)` (see [`sdk/types.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/plugins/sdk/types.ts)). The
...
Defined in [`sdk/permissions.ts`](https://github.com/arshad-shah/verql/blob/main/src/main/plugins/sdk/permissions.ts).
...
- **Zip-slip guard before extraction.** `installFromZip` lists the archive's
  ...
  traversal segment — **before** `unzip` writes anything to disk: defense-in-depth
  that doesn't trust `unzip`'s implicit stripping across versions/platforms.
  (Test: `zip-slip-guard.test.ts`.)
...
3. **No signature / publisher verification.** Verql does not yet verify a
   ...
   [`proposals/signed-plugins-and-registry.md`](https://github.com/arshad-shah/verql/blob/main/docs/proposals/signed-plugins-and-registry.md).
