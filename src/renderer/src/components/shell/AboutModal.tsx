...
export function AboutModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose() }}>
      {/* Full-bleed hero-split layout: zero out DialogContent's default padding
          (inline style so it wins over the package CSS) and clip the gradient
          panel to the dialog's rounded corners. The close (×) is the in-layout
          IconButton below, so the built-in one is disabled. */}
      <DialogContent
        size="lg"
