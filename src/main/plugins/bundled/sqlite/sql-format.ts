...
export const sqlExporter: RegisteredExporter = {
  ...
  execute(rows, columns, options) {
    ...
    if (options.includeSchema) {
      output += generateCreateTable(options.tableName, columns, SQLITE_QUOTE) + '\n'
    }
    if (rows.length === 0) {
      output += `-- No data in ${options.tableName}\n`
      return output
    }
    const colNames = columns.map(c => quoteIdentifier(c.name, SQLITE_QUOTE)).join(', ')
    const qTable = quoteIdentifier(options.tableName, SQLITE_QUOTE)
