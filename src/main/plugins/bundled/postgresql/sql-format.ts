...
export const sqlExporter: RegisteredExporter = {
  ...
  appliesTo: (t) => t === 'postgresql' || t === 'postgres',
  execute(rows, columns, options) {
    let output = ''
    if (options.includeSchema) {
      output += generateCreateTable(options.tableName, columns, PG_QUOTE) + '\n'
    }
    if (rows.length === 0) {
      output += `-- No data in ${options.tableName}\n`
      return output
    }
    const colNames = columns.map(c => quoteIdentifier(c.name, PG_QUOTE)).join(', ')
    const qTable = quoteIdentifier(options.tableName, PG_QUOTE)
